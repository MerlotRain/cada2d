...
RSpline::RSpline() :
    m_degree(3), m_periodic(false), m_dirty(true), m_updateInProgress(false), m_length(RNANDOUBLE) {
}
...
RSpline::RSpline(const std::vector<RVector>& controlPoints, int degree) :
    m_controlPoints(controlPoints), m_degree(degree), m_periodic(false), m_dirty(true), m_updateInProgress(false), m_length(RNANDOUBLE) {
...
std::vector<RSpline> RSpline::createSplinesFromArc(const RArc& arc) {
    ...
    for (double totalAngle = std::min(twoPI, std::fabs(endAngle - startAngle)); totalAngle > EPSILON; ) {
...
RSpline RSpline::createBezierFromSmallArc(double r, double a1, double a2) {
    ...
    std::vector<RVector> ctrlPts = {RVector(
            r * cos(a1),
            r * sin(a1)
           )
        , RVector(
...
RPolyline RSpline::toPolyline(int segments) const {
    ...
    std::vector<std::shared_ptr<RShape> > lineSegments = getExplodedBezier(segments);
...
//            exploded.append(std::shared_ptr<RShape>(line));
        }
        prev = p1;
...
std::vector<std::shared_ptr<RShape> > RSpline::getExplodedBezier(int segments) const {
...
void RSpline::appendToExploded(const RLine& line) const {
    ...
    if (!m_exploded.empty()) {
        ...
        std::shared_ptr<RLine> prev = std::dynamic_pointer_cast<RLine>(m_exploded.back());
        if (prev) {
...
std::vector<std::shared_ptr<RShape> > RSpline::getExplodedWithSegmentLength(double segmentLength) const {
    std::vector<std::shared_ptr<RShape> > ret;
    std::vector<RSpline> bezierSegments = getBezierSegments();
    for (int i=0; i<bezierSegments.size(); i++) {
...
std::vector<RVector> RSpline::getPointsWithDistanceToEnd(double distance, int from) const {
    std::vector<RVector> ret;
...
bool RSpline::reverse() {
    ...
    if (!isClosed()) {
        for (k = 0; k < m_controlPoints.size()/2; k++) {
            m_controlPoints.swap(k,m_controlPoints.size()-(1+k));
        }
        for (k = 0; k < m_fitPoints.size()/2; k++) {
            m_fitPoints.swap(k,m_fitPoints.size()-(1+k));
        }
        double t;
        int i, j;
        ...
        if (hasFitPoints()) {
            ...
            m_fitPoints.prepend(m_fitPoints.takeLast());
        }
        else {
            for (k = 0; k < m_controlPoints.size()/2; k++) {
                m_controlPoints.swap(k,m_controlPoints.size()-(1+k));
...
std::vector<RSpline> RSpline::getSegments(const std::vector<RVector>& points) const {
...
std::vector<RVector> RSpline::getDiscontinuities() const {
    ...
    if (isValid()) {
        for (int c=0; c<=11; c++) {
            ...
            do {
                double t;
                found = m_curve.GetNextDiscontinuity((ON::continuity)c, t0, t1, &t);
                if (found) {
...
void RSpline::updateFromControlPoints() const {
    ...
    if (m_periodic && !hasFitPoints()) {
        ...
        m_curve.CreatePeriodicUniformNurbs(3, getOrder(), m_controlPoints.size(), points);
        delete[] points;
    }
    ...
    // open or from fit points:
        ...
        bool knotCondition = (m_knotVector.size() == getOrder() + m_controlPoints.size() - 2);
        //knotCondition = true;
        ...
        if (m_knotVector.empty() || !knotCondition) {
            ...
            ON_MakeClampedUniformKnotVector(getOrder(), m_controlPoints.size(), knot);
            for (int i=0; i<si; ++i) {
...
std::vector<RSpline> RSpline::getBezierSegments(const RBox& queryBox) const {
    ...
    if (ctrlCount>0) {
        ON_NurbsCurve* dup = dynamic_cast<ON_NurbsCurve*>(m_curve.DuplicateCurve());
        if (dup==NULL) {
            return ret;
        }
...
bool RSpline::trimStartPoint(const RVector& trimPoint, const RVector& clickPoint, bool extend) {
    ...
    std::vector<RSpline> splines = splitAtPoints(std::vector<RVector>() << trimPoint);
    if (splines.size()>1) {
...
std::vector<RSpline> RSpline::splitAtPoints(const std::vector<RVector>& points) const {
...
std::vector<RSpline> RSpline::splitAtParams(const std::vector<double>& params) const {
...
std::vector<std::shared_ptr<RShape> > RSpline::splitAt(const std::vector<RVector>& points) const {
    ...
    for (int i=0; i<keys.size(); i++) {
        std::vector<RVector> values = sortable.values(keys[i]);
    ...
    for (int i=0; i<subSplines.size(); i++) {
        ret.append(std::shared_ptr<RShape>(subSplines[i].clone()));
    }
    return ret;
}
